...
export class ItemDetails extends DetailDialog {
    ...
    render(item,{source='',requirements='',requirementsLabel='领取条件',instanceGuid,owned=false}={}){
        ...
        if(item.slot){
            ...
            for(const row of equipmentAttributes(item,preview,c,instanceGuid))attributes.append(el('span','',`${row.label} ${signedAttribute(row.value)}${row.unit}`));
        ...
        if(requirements)this.body.append(el('p','equipment-source',`${requirementsLabel}：${requirements}`));
