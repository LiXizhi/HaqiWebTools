...
export function prepareQuestJournal(root) {
    ...
    fs.writeFileSync(path.join(root,'data/adventure/quest-journal.json'),JSON.stringify(payload)+'\n');
    fs.writeFileSync(path.join(root,'data/adventure/quest-runtime.json'),JSON.stringify(runtime)+'\n');
    return payload;
}
