...
import {magicStarStatus,magicStarWeek} from './adventure_magic_star_core.js';
...
export function renderMembership(body,model,cb,ui) {
    ...
    function select(key){
        ...
        if(key==='attributes'){
            detail.append(el('h3','','魔法星属性加成'),el('p','muted','原版数值一览；战斗属性加成暂未开放。'));
            ...
            detail.append(el('h3','','魔法星独有功能'),el('p','','已开放：商城会员专属商品、专属左手法杖、每周仙豆。'),el('p','','法杖需同时达到魔法星和角色等级要求，每个角色每件领取一次。已有同款法杖不重复发放。'),el('p','muted','原版属性加成、魔法星环绕表现和魔法口袋暂未开放。'),button('前往商城',()=>cb.panel('shop'),'primary'));
        }else{
            ...
            const list=el('div','magic-star-weekly-list');for(const row of config.levels)list.append(el('div',star.level===row.level?'current':'',`${row.level} 级`,el('strong','',`${row.weekly_money} 仙豆`)));detail.append(list,el('h3','','魔法口袋'),el('p','muted','神秘礼物暂未开放，敬请期待。'));
        }
    }
