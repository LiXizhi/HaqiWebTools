...
test('import preview presents differences safely and only confirms on explicit action', t=>{
    ...
    root.all().find(n=>n.textContent==='查看哈奇账号').onclick();assert.equal(reads,1);
    renderOriginalImport(root,{busy:'正在读取背包…',preview:null,error:''},cb);
    assert.equal(root.all().some(n=>n.textContent==='导入角色'),false);
    ...
    assert.equal(root.all().find(n=>n.textContent==='导入角色').disabled,true);
    assert.ok(root.all().some(n=>n.tag==='details'&&!n.open));
    assert.equal(root.all().filter(n=>n.attributes['aria-label']?.startsWith('预览')).length,2);
    ...
    assert.equal(root.all().find(n=>n.textContent==='导入角色').disabled,false);
    root.all().find(n=>n.textContent==='导入角色').onclick();assert.equal(confirmed,1);
