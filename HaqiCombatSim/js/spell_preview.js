...
$('seek').oninput=()=>{if(!card)return;elapsed=Number($('seek').value)/1000*effectDuration(assets.effects,card,$('reduced').checked);playing=false;$('play').textContent='播放';};
function frame(now){
    ...
    if(card){
        const spec=spellEffect(assets.effects,card),duration=effectDuration(assets.effects,card,$('reduced').checked);
        ...
        assets.tile(ctx,'sprites',10,from.x-size/2,from.y-size,size,size);
        for(const at of spec.area?targets:[to])assets.tile(ctx,spec.area&&spec.friendly?'sprites':'creatures',spec.area&&spec.friendly?10:1,at.x-size/2,at.y-size,size,size);
        fx.draw(ctx,{card,progress:p,from,to:targets[0],targets,center:{x:w*.5,y:h*.62},width:w,height:h,seed:7,reducedMotion:$('reduced').checked});
