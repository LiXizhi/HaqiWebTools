...
test('every chapter card has a supported effect and valid summon crop',()=>{
 ...
 const media=load('media');for(const r of config.frames){assert.ok(r[0]+r[2]<=media.entries.summons.width);assert.ok(r[1]+r[3]<=media.entries.summons.height);}
...
test('all effects draw finite, balanced canvas commands at every stage and direction',()=>{
 ...
 const fx=createSpellEffects({effects:config,draw:(...args)=>{draws++;for(const x of args.slice(2))assert.ok(Number.isFinite(x));}});
