// Keep the DOM/layout stable until release; downward drags discard on all pointers.
export function bindHandGesture(hand, { select, play, discard }) {
    ...
    function preview() {
        ...
        for(const {node} of g.cards){
            ...
            node.style.setProperty('--hand-drag-y',`${active?(g.rise<0?Math.min(120,-g.rise):-Math.min(120,Math.max(12,g.rise))):0}px`);
        }
    }
    function down(e) {
        if((e.pointerType!=='touch'&&e.button!==0)||e.isPrimary===false||gesture)return;
        ...
        e.preventDefault();
        gesture={id:e.pointerId,cards,card,baseY:e.clientY,rise:0,locked:false,touch:e.pointerType==='touch'};
    ...
    function move(e) {
        ...
        if(Math.abs(g.rise)>18)g.locked=true;
        if(Math.abs(g.rise)<10)g.locked=false;
        if(!g.locked&&g.touch){
    ...
    function up(e) {
        ...
        move(e);
        const {card,rise,touch}=gesture;
        const rect=hand.getBoundingClientRect();
        const inside=e.clientX>=rect.left-24&&e.clientX<=rect.right+24&&e.clientY<=rect.bottom+24;
        suppressClick(e);clear();
        const seq=Number(card.node.dataset.seq);
        if(rise<=-56&&discard){discard(seq);return;}
        if(inside)(touch&&rise>=56?play:select)(seq);
    }
