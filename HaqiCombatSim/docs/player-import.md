...
> 状态（2026-09-21）：**已接入 Haqi.html，先预览装备与卡包，最后确认创建独立新角色。** 下文旧日期的核查为历史记录，以本节实施范围为准；用户已明确允许产品接入。

## 产品入口与有限导入

角色创建第一步（起名字）和角色管理入口使用当前 Keepwork 登录会话读取原服；没有会话时先返回开始画面登录。多角色账号先选择原服角色；仍执行成年实名、角色号和账号一致性核验，不新增原服角色、不保存凭据。

仅请求背包 **0（穿戴）、1（装备）、24（已学牌）、25（符文）**，不会读取全部背包。依据为 `CombatInventorySubPage.lua:44`、`Item_CombatApparel.lua:159`、`CombatCardDeckSubPage.lua:290,1084,1502–1505`；卡组来自穿戴位置24的 `clientdata` 数字数组。独立测试页不传选择参数，保留原有全背包只读诊断。

`haqi_import_core.js` 对照现有物品、强化、宝石与技能数据转换并校验存档：装备保留原GUID对应实例、合法强化与镶嵌；不认识或不能安全转换的物品列入差异。卡组不合法时明确回退网页推荐卡组。等级按网页上限与经验曲线换算；任务、货币、VIP、图腾、原服宠物及其他成长不导入，网页等级技能与初始宠物保留并提示。

`haqi_import.js` 只在内存暂存，复用正式背包/卡包面板展示克隆存档。预览动作不保存，关闭或 Escape 可返回/取消；两个面板均查看后才能确认。确认前强制刷新身份并检查角色作用域，随后走现有角色存储创建独立角色（最多5个），不覆盖旧角色。本轮只保证本机落盘，云端仍走既有角色同步与冲突规则，不把本机成功冒充远端同步成功。

验收：35项导入/原服读取/视图/角色存储定向测试通过。真实产品入口及隔离合成数据页面验证背包与卡包、390px无横向溢出、确认前零提交、取消零提交、显式确认一次提交、身份变化拒绝；合成验收未写浏览器存档。本轮没有再次读取真实原服账号或创建真实导入角色。Vite打包通过；完整 `npm run build` 被已有 `camp.json` 地图快照过期门禁阻断，未改无关地图。

## 独立测试入口

使用现有开发服务打开 [HaqiImportTest.html](../HaqiImportTest.html)。页面仅在点击后连接 WebSocket 或登录 Keepwork，通过现有 API/OAuth 与原服协议读取人物和背包，显示 GUID、GSID、位置、数量及装备/卡包原始字段。可取消读取、清除显示；结果只留内存，不写本地或云端冒险存档，不加载主应用。

2026-09-21 修复认证回包兼容性：原版 `script/apps/GameServer/rest.lua:345–377` 转发认证结果时保留 `last_nid` / `new_nid`，原解析器只允许 `data` / `seq`，会把该回包误判为无效响应。现在校验并忽略这两个路由字段，仍拒绝未知字段、重复字段与可执行表达式；失败提示包含请求步骤，不输出账号数据或令牌。

补充修复：实际认证回包的 `last_nid` 使用 `~` 开头的临时连接编号；NPLRuntime 的 `NPLConnection.cpp` 明确约定此格式。解析器现在仅对 `last_nid` 接受 `~` 后跟数字，仍拒绝非法路由值。

定向协议/读取器/视图测试19项通过。真实 WebSocket Ping 返回协议版本26；使用已登录会话实测，AuthUser 回包已能解析，但服务器返回错误码438（原版 `paraworld.auth.lua` 定义为“未传递必要的用户凭证”）。页面现在显示请求步骤及安全错误码，不回显服务器内容。已对照 `KeepWorkLogin.agreeOauth` 的平台与OAuth字段；包装层还会从原客户端会话补充 `nid2`，但尚无证据确定438的具体缺失字段，不猜测或绕过认证。此为修复前实测；后续认证修复及完整读取结果见下段。尚不转换装备、卡组或创建新角色；不能将解析成功视为完整导入成功。

2026-09-21 后续已定位并修复438：原版 `MainLogin.lua:1010–1080` 在认证前调用 `Users.GetNIDByOtherAccountID`，由用户选角色，再将角色号作为 `nid2` 传入认证。网页原先缺失整个步骤。现已补齐：单角色自动使用，多角色在独立页选择；拒绝列表外角色，核验认证返回角色与选择一致，无角色时不创建。使用浏览器现有登录会话验证目标账号，未使用或保存聊天中的密码；完整实测成功读取46个背包、821条物品，页面显示“只读拉取完成”。20项定向测试通过，冒险存档与主应用均未改动。
...
- 新建分为起名字、选择抱抱龙、选择系别；仅起名字步骤提供小字链接：**“导入魔法哈奇角色”**（2026-09-21 用户调整）。保持手机可读、可点击和键盘可达。
