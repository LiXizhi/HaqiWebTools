...
天气沿用 `config/maps/generator.json` 的各地域kind、count、speed和wind：雪花、沙尘、余烬、薄雾、灰烬、萤光共用一张图集。粒子锚定世界坐标，角色移动时不跟着镜头走；仍只绘制当前屏幕内最多64个，不按岛屿面积增加。系统减少动态效果时停用天气。树木按视野剔除、脚底深度排序，阴影继续预烘焙。
