...
**导出不等于可玩。** 只有既有14个营地任务标记 `opening-adaptation`；其余670个为 `catalog-only`。尚未把原服活动、剧情脚本、任务奖励或复杂条件接入单人存档。不能直接把全部任务追加到现有线性教学任务执行器。
...
原14条教学任务使用真实进度、奖励详情和追踪；其余明确只读，废除任务可通过状态筛选查看。地区筛选、编号/名称搜索、20项分页和前置跳转均不修改存档。此前“未接入”的限定现在指任务执行与剧情玩法，目录展示已完成。
