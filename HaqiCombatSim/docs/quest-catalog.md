...
火鸟岛、寒冰岛、沙漠岛和幽暗岛不再只有居民：island-encounters.json 提供114处原模板单怪遭遇，其中101处支持挑战，13处保留缺失卡牌/未支持目标规则的禁用原因。按原世界和任务分类补齐模板，单怪阵容与道路位置是明确的二维单人改编。已接任务在胜利后依照原模板路径累计击杀与概率掉落；野外遭遇可重复挑战。任务追踪可跨岛寻找对应怪物，优先当前岛及任务所属岛。美术全部复用既有Boss和宠物WebP。

运行 npm run prepare:island-encounters 可刷新快照；更改地图、居民或任务目录后需要重新生成。构建会校验快照。此修复不代表原版所有Boss脚本已迁移。

2026-09-25续：按用户授权，6项缺失卡牌引用改用现有相近直伤牌（配置在config/monster-card-replacements.json），8处遭遇解除阻断，四岛109/114可挑战。旧火系数据不足，替换属于Web近似而非原数值恢复。原目录与副本保留原规则；岛屿改编模板用独立ID避免副本加载覆盖，任务仍按原source计数。其余5处受旧寒冰吸收、冰封或noshield阻断。

2026-09-25最终更新：用户授权近似替换剩余规则，旧寒冰吸收→600点单体吸收盾、冰封→寒冰单体眩晕、无盾选目标→最高仇恨。四岛114/114处均可挑战。配置仍统一维护在config/monster-card-replacements.json，保留卡牌和目标替换理由；不表示原版冰封或AI规则已经迁移。
