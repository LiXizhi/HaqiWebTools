...
import {rewardStatus} from './language_adventure_core.js';

export function createStoryChat({getState,commit,saveSettings=()=>{},openSettings=()=>{},voice,viewFactory=createLearningChatView}){
    let session=null,recordTask=null;
    const view=viewFactory({close,start,finish,cancel,hint,chinese,next,help,speak,settings:()=>{close();openSettings();}});
    ...
    function open(profile,story,portrait){
        close();const current=getState();session={profile,story,portrait,locale:current.save.languageLearning.target,showChinese:current.save.languageLearning.showChinese!==false,role:current.role,identity:current.identity,abort:new AbortController(),operation:0,index:0,messages:[],proof:[],hintLevel:3,hintsUsed:true,busy:false,recording:false,done:false,phase:'ready',attemptId:crypto.randomUUID(),status:'听问题，参考下方提示开口回答，也可以用自己的话表达。'};
        const status=rewardStatus(current.save,current.content,{id:story.rewardGroup,tier:'beginner'},'basic',Date.now());
        session.messages.push({role:'npc',text:story.opening},{role:'npc',text:story.turns[0].question});
        session.status+=status.amount?` 完成可得 ${status.amount} 奇豆。`:' 今日奖励已达上限，仍可练习。';paint();
        if(current.save.languageLearning.autoSpeak)void speak(story.turns[0].question[session.locale]);
    }
    ...
    async function finish(){
        ...
        try{
            ...
            if(s.index+1<s.story.turns.length){s.index++;s.hintLevel=3;s.hintsUsed=true;s.messages.push({role:'npc',text:s.story.turns[s.index].question});s.status='这句通过了，继续聊下一句。';}
            else{
                // Persist only after all three current-session microphone answers passed.
                const completion={course:{id:s.story.rewardGroup,tier:'beginner'},mode:'basic',locale:s.locale,attemptId:s.attemptId,now:Date.now(),score:100,spoken:true,story:{id:s.story.id,turnIds:s.story.turns.map(t=>t.id),proof:s.proof,hintsUsed:s.hintsUsed}};
                const result=commit(completion);s.done=true;s.messages.push({role:'npc',text:s.story.ending});s.status=result.amount?`交流完成，获得 ${result.amount} 奇豆。`:'交流完成，已记录口语练习。';
            }
