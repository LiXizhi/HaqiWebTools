...
function progressCard(save, label) {
    return el('article', 'cloud-progress', el('p', 'eyebrow', label), save
        ? el('div', '', el('h3', '', save.name), el('p', '', `${SCHOOL_NAMES[save.school]} · 等级 ${save.level} · ${Object.values(save.quests).filter(q => q.claimed).length} / 14 任务`), el('small', 'muted', `${save.zone === 'town' ? '哈奇小镇' : '魔法营地'}${save.pendingEncounter ? ' · 战斗中（恢复已保存的回合）' : ''}`))
