...
> 状态：**Phase 0~6 已实施完成**（2026-09-16）；其后冒险/发布阶段见文中各节。**2026-09-18 真实用户数据与异步 PVP 仅为计划需求，未实现。** 实施进度以本文件 checkbox、[qa-report.md](qa-report.md) 和 `devlog/` 为准。与原计划的偏差在各阶段末以「实际」标注。
...
两项产品需求已写入文档，**尚未实现**。调查范围为本仓库 `HaqiCombatSim/` 与兄弟目录 `HaqiCombatEmulator/`；不编造生产 API。
...
- 没有：匹配队列、选牌/整场过期时钟、本目录的 PvP 战报导出。
- 匹配规则、过期判负/托管、回放存哪：见 [async-pvp.md](async-pvp.md)。
