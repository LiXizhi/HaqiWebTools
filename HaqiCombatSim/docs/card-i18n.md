...
3. **teen**：~~海外是否包含 teen 数据集？~~ 已决（2026-09-26 用户约定）：内容收录只针对儿童版（kids），不为 teen 补录效果说明或翻译；teen 数据与分支保持现状。
...
## 范围约定（2026-09-26 用户约定）

卡牌效果说明、效果文案与 `data/adventure/locale/en.txt` 翻译补录等面向用户的内容工作，**只覆盖 `data/kids/`（儿童版）**；不移植、不补录、不翻译青年版（teen）内容。teen 数据集、引擎分支与既有 teen 专属 type（`Global2`、`Absorb_Adv`、`SingleAttackHP`、`AreaControl` 等）保持现状，不为其扩展内容完整性。
