...
export function createLearningChatView(cb){
    ...
    const chinese=button('隐藏中文',()=>cb.chinese(),'secondary small'),change=button('换个故事',()=>cb.next(),'secondary small');
    ...
    footer.append(hint,form,actions,caption,status);panel.append(header,log,footer);room.append(aside,panel);root.append(room);document.body.append(root);
    ...
    function message(row,s){
        ...
        if(translation&&(s.showChinese||s.locale!=='en'))content.append(el('p','camp-chat-translation',translation));
    ...
    return {
        ...
        render(s){
            ...
            if(key!==s.story.id){key=s.story.id;name.textContent=s.profile.name;role.textContent=s.profile.role;context.textContent=s.story.context;title.textContent=s.story.title;
                portrait.hidden=!s.portrait;if(s.portrait)portrait.src=s.portrait;form.hidden=true;
            }
            progress.textContent=s.done?'交流完成':`第 ${s.index+1} / ${s.story.turns.length} 轮`;
            chinese.textContent=s.showChinese?'隐藏中文':'显示中文';chinese.setAttribute('aria-pressed',String(!s.showChinese));
            ...
            hints.textContent=s.hintLevel?'收起提示':'显示提示';hints.setAttribute('aria-expanded',String(!hint.hidden));
            if(!hint.hidden){const turn=s.story.turns[s.index];hint.replaceChildren(el('strong','','回答提示'),el('p','',turn.hint));
                if(s.hintLevel>=2)hint.append(el('p','',turn.pattern.replace(/\{\w+\}/g,'…')));
                if(s.hintLevel>=3){hint.append(el('p','camp-chat-original',turn.answer[s.locale]));if(s.showChinese||s.locale!=='en')hint.append(el('p','camp-chat-translation',turn.answer[s.locale==='en'?'zh-CN':'en']));const sample=button('听示范',()=>cb.speak(turn.answer[s.locale]),'secondary small');sample.disabled=s.busy||s.recording;hint.append(sample);}
            }
            mic.disabled=s.done||(s.busy&&!s.recording&&s.phase!=='connecting');mic.classList.toggle('is-recording',!!s.recording);mic.textContent=s.recording?'结束':s.phase==='connecting'?'连接中':'录音';
            ...
            status.textContent=s.status;caption.textContent=s.recording?'正在录音 · 松开发送，滑出取消':'按住对话，松开发送 · 点击录制，再点结束';
