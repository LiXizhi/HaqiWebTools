...
export function createAdventure(content, { name = '小哈奇', school = 'fire', appearance = 'boy', seed = 530 } = {}) {
...
export function recommendedDeck(save, content) {
    ...
    const priority = [3,5,1,0,2,4,6].map(i => lessons[i]).filter(Boolean);
...
export function applyAction(save, content, action) {
    ...
    case 'hatch':
        ...
        save.pet = { itemId: content.pet.itemId, name: content.pet.name, xp: 0, level: 0 }; syncGoals(save,content); break;
    case 'feed': {
        ...
        save.pet.level = petLevel(save.pet.xp,content); syncGoals(save,content); break;
    }
    case 'deck':
...
export function settleEncounter(save,content,battle) {
    ...
    const encounter = content.encounters.find(e => e.id === pending.encounterId), monster = content.monsters[encounter.monsterId];
...
export function parseSave(raw,content) {
    ...
    if (s.pendingEncounter) {
        ...
        assert(content.encounters.some(e => e.id === s.pendingEncounter.encounterId && e.zone === s.zone) && Array.isArray(s.pendingEncounter.decisions) && s.pendingEncounter.decisions.length <= 200 && Number.isInteger(s.pendingEncounter.seed),'存档战斗无效');
    }
