...
普通NPC描述和连续剧情对白在学习模式下默认同时显示目标语言与母语（中英试点为英文在上、中文在下），沿用“隐藏中文”偏好。点击目标语言台词或末尾喇叭使用设置中的音色朗读；点击母语译文或末尾“词义映射”，固定调用keepwork-lite标注双语词义对应（需要Keepwork登录）。同义用同色，近似对应使用邻近色及虚线下划线，名词/动词/修饰词/功能词分色系；未对应片段保留原色。模型只提供分段标注，逐字校验后以纯文本渲染，不改动原句。同一窗口成功后复用结果，关闭时取消未完成请求及音频；点击台词不会触发任务或商店按钮。未开启学习模式仍使用原逐字对白。缺失译文仅显示已有原文，不伪造目标语言内容。
